...
class ScrapingBeeProvider:
    ...
    def _build_multi_city_results_scenario(self, *, deep: bool = False) -> dict[str, object]:
        ...
        wait_for_search_settle_script = (
            ...
            "return body?body[0]:'';"
            "};"
            "const summaryReady=()=>['cheapest','best'].every(label=>"
            "Array.from(document.querySelectorAll('button,a,[role=\"button\"],div,span'))"
            ".some(el=>visible(el)&&new RegExp('^'+label+'(?:\\\\s|$)').test(norm(el.innerText||el.getAttribute('aria-label')||''))));"
            "let stableSince=now();"
            "let lastCount=countText();"
            "let lastSummary=summaryReady();"
            "const started=now();"
            "const tick=()=>{"
            "const currentCount=countText();"
            "const currentSummary=summaryReady();"
            "const moving=progressVisible();"
            "if(currentCount!==lastCount||currentSummary!==lastSummary){"
            "lastCount=currentCount;lastSummary=currentSummary;stableSince=now();"
            "}"
            "if(currentSummary&&currentCount&&now()-stableSince>=2500){done();return;}"
            "if(!moving&&currentSummary&&now()-stableSince>=2000){done();return;}"
            "if(now()-started>=12000){done();return;}"
            "setTimeout(tick,500);"
            "};"
            "tick();"
            "}))()"
